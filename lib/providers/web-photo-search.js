...
export function buildWebPhotoSearchLinks({ vin, year = '', make = '', model = '' }) {
  ...
  return [
    ...
    },
    {
      ...
      url: `https://autohelperbot.com/en/car/${encodedVin}`,
      note: 'Auction history and car details when indexed. Opens in new tab.'
    },
    {
      label: '💰 Bid.Cars - VIN auction history',
      source: 'bid-cars-search',
      url: `https://bid.cars/en/bidfax?vin=${encodedVin}`,
      note: 'Copart/IAAI auction history and bid data when available. Opens in new tab.'
    }
  ];
}
