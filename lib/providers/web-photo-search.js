...
export function buildWebPhotoSearchLinks({ vin, year = '', make = '', model = '' }) {
  ...
  return [
    ...
    {
      label: 'Search AutoHelperBot by VIN',
      source: 'autohelperbot-search',
      url: `https://autohelperbot.com/en/search?query=${encodedVin}`,
      note: 'Public indexed auction/history leads may include photos when a matching record exists.'
    },
    {
      label: 'Search Bid.Cars by VIN',
      source: 'bid-cars-search',
      url: `https://bid.cars/en/search?q=${encodedVin}`,
      note: 'Auction leads may include sale records and photo galleries when indexed.'
    }
  ];
}
