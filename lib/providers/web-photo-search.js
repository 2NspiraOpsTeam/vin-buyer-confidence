...
export function buildWebPhotoSearchLinks({ vin, year = '', make = '', model = '' }) {
  ...
  // Generate a pseudo-random ID for AutoHelperBot URL
  const autoHelperId = Math.floor(10000000 + Math.random() * 90000000);
  ...
  return [
    ...
    {
      label: '🚗 AutoHelperBot - VIN car search',
      source: 'autohelperbot-search',
      url: `https://autohelperbot.com/en/car/${normalizedVin}_${autoHelperId}`,
      note: 'Auction history and car details when indexed. Opens in new tab.'
    },
    {
      label: '💰 Bid.Cars - Search by VIN',
      source: 'bid-cars-search',
      url: `https://bid.cars/en/search/results?search-type=typing&query=${encodedVin}&status=All&make=All&model=All&year-from=1900&year-to=2027&auction-type=All`,
      note: 'Search results for this VIN. Opens in new tab.'
    }
  ];
}
