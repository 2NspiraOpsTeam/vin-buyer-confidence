...
export function buildWebPhotoSearchLinks({ vin, year = '', make = '', model = '' }) {
  ...
  // Build review links for common automotive review sites
  const reviewLinks = [];
  if (make && model) {
    ...
    reviewLinks.push({
      label: '🚗 CarGurus - ' + make + ' ' + model,
      source: 'cargurus-review',
      url: `https://www.cargurus.com/Cars/searchresults?make=${makeSlug}&model=${modelSlug}`,
      note: 'User reviews and pricing for ' + make + ' ' + model
    });

    reviewLinks.push({
      label: '🚗 Edmunds - ' + make + ' ' + model + ' Reviews',
      source: 'edmunds-review',
      url: `https://www.edmunds.com/${makeSlug}/${modelSlug}/`,
      note: 'Expert reviews and ratings'
    });

    reviewLinks.push({
      label: '🚗 Kelley Blue Book - ' + make + ' ' + model,
      source: 'kbb-review',
      url: `https://www.kbb.com/${makeSlug}/${modelSlug}/`,
      note: 'Expert reviews and pricing'
    });

    reviewLinks.push({
      label: '🚗 MotorTrend - ' + make + ' ' + model,
      source: 'motortrend-review',
      url: `https://www.motortrend.com/search/?q=${encodedMake}+${encodedModel}`,
      note: 'Reviews and road tests'
    });

    reviewLinks.push({
      label: '🚗 Consumer Reports - ' + make + ' ' + model,
      source: 'consumerreports-review',
      url: `https://www.consumerreports.org/cars/${makeSlug}/${modelSlug}/`,
      note: 'Reliability and testing data (may require subscription)'
    });
  }
