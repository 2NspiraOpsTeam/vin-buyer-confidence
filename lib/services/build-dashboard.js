...
import { getListingPageSnapshot } from '../providers/listing-page.js';
import { getMonroneySnapshot } from '../providers/monroney.js';
import { getVehicleHistorySnapshot } from '../providers/history.js';
import { getVinPresenceSnapshot } from '../providers/vin-presence.js';
...
export async function buildVehicleDashboard({ vin, askingPrice = null, mileage = null, listingUrl = '', condition = 'good' }) {
  const [nhtsaIdentity, recallRows, marketcheckRaw, autodevRaw, listingPageRaw, monroney, historyRaw, vinPresenceRaw] = await Promise.all([
    getNhtsaIdentity(vin),
    getNhtsaRecalls(vin),
    getMarketCheckListingSnapshot({ vin }),
    getAutoDevListingSnapshot({ vin }),
    getListingPageSnapshot({ listingUrl }),
    getMonroneySnapshot({ vin }),
    getVehicleHistorySnapshot({ vin }),
    getVinPresenceSnapshot({ vin })
  ]);
  ...
  const mergedPhotos = dedupeBy(
    [...(listingPageRaw?.photos || []), ...(marketcheck?.photos || []), ...(autodev?.photos || [])],
    item => item
  );
  ...
  const normalizedListingEvidence = normalizeListingEvidence(
    {
      ...
      photoSource: listingPageRaw?.photos?.length
        ? 'listing-page'
        : autodevRaw?.status === 'live' && autodev?.photos?.length
          ? 'auto.dev'
          : marketcheckRaw?.status === 'live' && marketcheck?.photos?.length
            ? 'marketcheck'
            : 'not_available',
      photoStatus: listingPageRaw?.photos?.length
        ? listingPageRaw.status
        : mergedPhotos.length
          ? 'provider_images_found'
          : 'not_available',
      photoNote: listingPageRaw?.photos?.length
        ? listingPageRaw.note
        : 'No VIN-specific listing photos were returned. Connect Auto.dev/MarketCheck or provide a listing URL with readable image metadata.'
    },
  ...
  return {
    ...
    provenance: [
      ...
      { source: 'listing-page', status: listingPageRaw?.status || 'not_provided' },
      ...
      { source: 'vin-presence', status: vinPresenceRaw?.status || 'not_configured' }
    ],
    gating: {
