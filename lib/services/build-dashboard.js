...
export async function buildVehicleDashboard({ vin, askingPrice = null, mileage = null, listingUrl = '', condition = 'good' }) {
  ...
  return {
    ...
    listingEvidence: normalizeListingEvidence({ ...marketcheck, ...autoDev, photos: mergedPhotos }, { listingUrl, askingPrice, mileage, source: 'provider_merge' }),
    marketComps: normalizeMarketComps(mergedComps),
    pricingContext: {
      askingPrice,
    ...
    provenance: [
      ...
      { source: 'auto.dev', status: autodev?.status || 'scaffolded' },
